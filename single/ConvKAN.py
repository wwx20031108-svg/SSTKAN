...
from fast_kan import Fast_KANLinear, FastKAN
...
class ConvKAN(nn.Module):
    ...
    def forward(self, x):
        ...
        assert x.dim() == 4
        #print(f"Expected in_channels: {self.in_channels}, Received in_channels: {in_channels}")
        assert in_channels == self.in_channels, f"Input channels {in_channels} do not match expected {self.in_channels}"
